...
const onReady = (key) => {
  ...
  axios.post(
    `${apiUrl}/api/services/persistent_notification/dismiss`,
    {
...
const onQr = (qr, key) => {
  ...
  code.on("readable", function () {
    ...
    axios.post(
      `${apiUrl}/api/services/persistent_notification/create`,
      {
...
const onMsg = (msg, key) => {
  axios.post(
    `${apiUrl}/api/events/new_whatsapp_message`,
    { clientId: key, ...msg },
    {
      headers: {
        Authorization: `Bearer ${token}`,
...
const onPresenceUpdate = (presence, key) => {
  axios.post(
    `${apiUrl}/api/events/whatsapp_presence_update`,
